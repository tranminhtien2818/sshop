...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    @Query(value = "select p from Product p where p.category.id = ?1 and p.tradeMark.id = ?2")
    ...
    @Query(value = "SELECT * from product where category_id = ?1 and id != ?2 limit 4", nativeQuery = true)
    ...
    @Query("select p from Product p where p.price >= ?1 and p.price <= ?2 and p.category.id = ?3")
    public Page<Product> searchFull1(Double small, Double large, Long cateId, Pageable pageable);

    @Query("select p from Product p where p.price >= ?1 and p.price <= ?2 and p.tradeMark.id = ?3")
    public Page<Product> searchFull2(Double small, Double large, Long trademark, Pageable pageable);

    @Query("select p from Product p where p.price >= ?1 and p.price <= ?2 and p.tradeMark.id = ?3 and p.category.id = ?4")
    ...
    @Query(value = "(SELECT p.* from product p where p.category_id = ?2 ORDER BY p.id DESC limit ?1) " +
            "UNION (SELECT p.* from product  p where p.category_id = ?3 ORDER BY p.id DESC limit ?1) " +
            "UNION (SELECT p.* from product  p where p.id = ?5) " +
            "UNION (SELECT p.* from product  p where p.id = ?4) " +
            "UNION (SELECT p.* from product  p where p.category_id != ?2 and p.category_id != ?3 ORDER BY p.id DESC limit ?1)", nativeQuery = true)
    public Set<Product> getByCate(Integer size, Long cate1, Long cate2, Long pro1, Long pro2);

    @Query(value = "(SELECT p.* from product p where p.name like ?2 ORDER BY p.id DESC limit ?1) " +
            "UNION (SELECT p.* from product  p where p.name like ?3 ORDER BY p.id DESC limit ?1) " +
            "", nativeQuery = true)
